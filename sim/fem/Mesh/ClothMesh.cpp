...
ClothMesh::ClothMesh() {
	n=12; m=8;
	double nSz = 75.0, mSz = 50.0;
	double thick = 8.0;
	GridMesh* m1 = new GridMesh(n,m,nSz,mSz,Eigen::Vector3d(8.89406-mSz/2,141.677-nSz,-88.409+thick+nSz*0.1),Eigen::Vector3d(0.0,1.0,-0.1),Eigen::Vector3d(1.0,0.0,0.0));
	GridMesh* m2 = new GridMesh(n,m,nSz,mSz,Eigen::Vector3d(8.89406-mSz/2,141.677,-88.409-thick),Eigen::Vector3d(0.0,-1.0,-0.2),Eigen::Vector3d(1.0,0.0,0.0));
	...
	for(int i=1;i<=n-1;i++) {
		...
		sq.push_back((i-1)*m);
		sq.push_back(i*m);
		sq.push_back((2*n-1-i)*m);
		sq.push_back((2*n-i)*m);
